...
const (
	READ_LEN = Dm_build_371
)

type iOffRowBinder interface {
	read(buf *Dm_build_1129)
	isReadOver() bool
	getObj() interface{}
}
...
func newOffRowBytesBinder(obj []byte, encoding string) *offRowBytesBinder {
	...
	binder.offRow = binder.buffer.Dm_build_1134() > Dm_build_368
	return binder
}

func (b *offRowBytesBinder) read(buf *Dm_build_1129) {
	if b.buffer.Dm_build_1134() > 0 {
		buf.Dm_build_1166(b.buffer)
	} else if !b.readOver {
		var obj = b.obj.([]byte)
		buf.Dm_build_1155(obj, 0, len(obj))
		b.readOver = true
	}
}
...
func (b *offRowBlobBinder) read(buf *Dm_build_1129) {
	if b.buffer.Dm_build_1134() > 0 {
		buf.Dm_build_1166(b.buffer)
	} else if !b.readOver {
		...
		buf.Dm_build_1155(bytes, 0, len(bytes))
	}
}
...
func (b *offRowClobBinder) read(buf *Dm_build_1129) {
	if b.buffer.Dm_build_1134() > 0 {
		buf.Dm_build_1166(b.buffer)
	} else if !b.readOver {
		...
		var bytes = Dm_build_853.Dm_build_1063(str, b.encoding, nil)
		b.position += readLen
		if b.position == int32(totalLen) {
			b.readOver = true
		}
...
func (b *offRowReaderBinder) read(buf *Dm_build_1129) {
	if b.buffer.Dm_build_1134() > 0 {
		buf.Dm_build_1166(b.buffer)
	} else if !b.readOver {
		var err error
...
func (b *offRowReaderBinder) readAll() []byte {
	var byteArray = Dm_build_1133()
	b.read(byteArray)
	for !b.readOver {
		b.read(byteArray)
	}
	return byteArray.Dm_build_1176()
}
